...
foreach x of varlist `credit_vars' {
	...
	qui reg `x' c.t##i.medicare##i.state_fips [aweight=population] if inrange(age, 62,68) & age != 68, robust
	replace medicare = 0
...
preserve 
collapse *_upper *_lower pctui_young [aweight=population] if age == 65, by(state state_fips state_name)
...
foreach x in `credit_vars' { 
	...
	qui reg `x'_upper `x'_lower, robust
	local b = string(_b[`x'_lower], "%9.3f")


	twoway (scatter `x'_upper `x'_lower , mlab(state) mlabpos(0) m(i)), ///
	  name( b, replace)  legend(off) xtitle("State Effects without Medicare") ytitle("State Effects with Medicare") ///
	  text(`y_location' `x_location' "{&beta} = `b'" "{&theta} = `medicare_ratio'" )
	graph export "../graphs/`x'_theta1_scatter.pdf", replace

	twoway (scatter `x'_upper `x'_lower pctui , mlab(state state) mlabpos(0 0) m(i i)), ///
	  name( b, replace)  legend(off) xtitle("Near-elderly uninsurance rate") ytitle("State Effects")
	graph save "../graphs/`x'_pctui_scatter.gph", replace
	graph export "../graphs/`x'_pctui_scatter.pdf", replace
}

save ../data/theta1_state_data, replace
