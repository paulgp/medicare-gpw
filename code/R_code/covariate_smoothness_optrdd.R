...
main_data <- vroom("~/Dropbox/Research/GPW/data/final_county_level.csv",
                   delim = ",",
                   col_types = cols(
                     quarter = col_double(),
                     state = col_character(),
...
reg <- main_data %>% select(one_of(outcome_vars), age, population,  census_code, state, quarter) %>% 
  mutate(age = age - 1) %>%
  filter(age > 55 & age < 75) %>% 
  filter(age != 65) %>% 
  group_by(age,  census_code, state) %>%
  gather(variable, outcome, -age, -population, -census_code, -state, -quarter) %>%
  group_by(age,  census_code, state, variable) %>%
  summarise(outcome = weighted.mean(outcome, population),
            population = sum(population))  %>%
  mutate(W = as.numeric(age > 65)) 

# Calculate max B
overall_reg <- main_data %>% select(one_of(outcome_vars), age, population,  census_code, state, quarter) %>% 
  mutate(age = age - 1) %>%
  filter(age > 55 & age < 75) %>% 
  filter(age != 65) %>% 
  gather(variable, outcome, -age, -population, -census_code, -state, -quarter) %>%
...
max_B = overall_reg %>%
  group_by(variable) %>%
  summarise(max_B = max(abs(B), na.rm=TRUE))
...
for (i in seq(1,length(outcome_vars))) {
  ...
  out.1 = optrdd(X=data_input$age, Y=data_input$outcome, W=data_input$W, 
                 max.second.derivative =max_B$max_B[max_B$variable == varname], 
...
for (i in seq(1,length(outcome_vars))) {
  ...
  ggsave(paste0("~/Dropbox/GPW/graphs/", varname, "optrdd_impute_test.pdf"))
}
