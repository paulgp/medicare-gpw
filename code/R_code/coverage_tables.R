...
main_data <- read_csv("~/Dropbox/Research/GPW/derived/import_and_collapse_ACS/output/ACS_microdata_collapsed_by_age_county_post2008.csv")

outcome_vars <- c("has_insurance", "has_private_insurance", "has_medicaid", "has_medicare", 
                  "has_multiple")

outcome_var_labels <- c("Has health insurance", "Has private health insurance",
                        "Has Medicaid", "Has Medicare", "Has multiple sources of coverage")
...
#  mutate(age = age - 1) %>%
  mutate(has_insurance = has_insurance*100, has_private_insurance= has_private_insurance*100, has_medicaid = has_medicaid*100,
         has_medicare = has_medicare*100, has_multiple = has_multiple*100) %>%
...
for (i in seq(1,length(outcome_vars))) {
  ...
  ggplot(data=data_input, aes(x = age, y = outcome, color = as.factor(W))) +
    ...
    geom_point(data = predicted_val, aes(y = outcome, x = age), color = c("#0072b2") ) +
    theme_minimal() +
...
fileConn<-file("~/Dropbox/Research/GPW/tables/insurance_rd_optrdd.tex")
