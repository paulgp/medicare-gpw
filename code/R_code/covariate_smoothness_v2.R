...
reg <- main_data %>% select(one_of(outcome_vars), age, population,  countyfip) %>% 
  ...
  group_by(age,  countyfip) %>%
  gather(variable, outcome, -age, -population, -countyfip) %>%
  group_by(age,  countyfip, variable) %>%
  summarise(outcome = weighted.mean(outcome, population),
            population = sum(population))  %>%
  mutate(W = as.numeric(age >= 65)) 

# Calculate max B
overall_reg <- main_data %>% select(one_of(outcome_vars), age, population,  countyfip) %>% 
  mutate(own_dwelling = own_dwelling*100, married= married*100, employed = employed*100,
         fam_size_1 = fam_size_1*100, fam_size_2 = fam_size_2*100, fam_size_more = fam_size_more*100,
         female = female*100) %>%
  filter(age >= 50 & age <= 80) %>% 
