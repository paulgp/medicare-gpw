...
keep if inrange(age, 60,64)
collapse `credit_vars'   (rawsum) population  [aw=population ], by(state)

statastates, abbreviation(state)
keep if _merge == 3

graph bar q_coll_12, over(state, sort(1) label(labsize(tiny) angle(90)))
graph export "`path'/graphs/q_coll_12_by_state_60_to_64.pdf", replace

graph bar balance_ttl_delinq, over(state, sort(1) label(labsize(tiny) angle(90)))
graph export "`path'/graphs/ttl_delinq_by_state_60_to_64.pdf", replace
...
graph bar q_coll_12, over(state, sort(1) label(labsize(tiny) angle(90)))
graph export "`path'/graphs/q_coll_12_by_state_21_to_64.pdf", replace

graph bar balance_ttl_delinq, over(state, sort(1) label(labsize(tiny) angle(90)))
graph export "`path'/graphs/ttl_delinq_by_state_21_to_64.pdf", replace
